...
public class RapidFireMove : AncientAspidMove
{
    ...
    public override IEnumerator DoMove()
    {
        ...
        //var playerVector = (Player.Player1.transform.position - Boss.Head.transform.position);

        //var referencePosition = Boss.Head.transform.position;
        //var toPlayerVector = Vector3.right * playerVector.magnitude;
        ...
        for (int i = 0; i < rowsToShoot; i++)
        {
            ...
            //currentAngle = GetTargetAngle(0f);

            float fireRate = (1f / shotsPerRow) * rowTime;

            float fireTimer = 0f;

            //fireTimer = new Vector2(0f, fireRate).RandomInRange();
            /*if (travelingLeft)
            {
            ...
            for (float t = 0f; t < rowTime; t += Time.deltaTime)
            {
                ...
                //currentAngle = GetTargetAngle(t / rowTime);
                var targetRotation = Quaternion.Slerp(from, to, t / rowTime);
                var angle = laserMove.CalculateLaserRotation(targetRotation);
                laserMove.SetLaserRotation(angle.main, angle.extra);
                var spriteIndex = laserMove.GetHeadIndexForAngle(angle.main);
