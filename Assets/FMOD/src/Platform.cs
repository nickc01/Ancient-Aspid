...
namespace FMODUnity
{
    ...
    public abstract class Platform : ScriptableObject, ISerializationCallbackReceiver
    {
        ...
        private void OnValidate()
        {
            ...
            {
                EXTRACT = false;
                var newInstance = (Platform)ScriptableObject.CreateInstance(GetType());

                EditorJsonUtility.FromJsonOverwrite(EditorJsonUtility.ToJson(this), newInstance);
                newInstance.name = name;

                AssetDatabase.CreateAsset(newInstance, $"Assets/{name}_{GetType().Name}.asset");
            }
        }
#endif
        ...
        public BinaryFileInfo CreateFileInfo(FileRecord record, BuildTarget buildTarget, BinaryType binaryType)
        {
        ...
        [SerializeField]
        string properties_json_data = "";

        [SerializeField]
        List<UnityEngine.Object> properties_object_data = new List<UnityEngine.Object>();

        [SerializeField]
        string threadAffinities_json_data = "";

        [SerializeField]
        List<UnityEngine.Object> threadAffinities_object_data = new List<UnityEngine.Object>();

        public void OnBeforeSerialize()
        {
#if UNITY_EDITOR
            WeaverSerializer.Serialize(Properties, out properties_json_data, out properties_object_data);
            WeaverSerializer.Serialize(threadAffinities, out threadAffinities_json_data, out threadAffinities_object_data);
#endif
        }
        ...
        {
#if !UNITY_EDITOR
            Properties = WeaverSerializer.Deserialize<PropertyStorage>(properties_json_data, properties_object_data);
            threadAffinities = WeaverSerializer.Deserialize<PropertyThreadAffinityList>(threadAffinities_json_data, threadAffinities_object_data);
#endif
        }
