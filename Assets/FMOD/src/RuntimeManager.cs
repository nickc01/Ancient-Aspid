...
retry:
        ...
        public static void LoadBankFile(Stream bankData, bool loadSamples, string bankID)
        {
            ...
            else
            {
...

                {
                    ...
                    using (var uStream = new UnmanagedUtilities.UnmanagedStreamWrapper(bankData, out var ptr, out var length))
                    {

                        //try
                        //{
                        /*if (bankData is UnmanagedMemoryStream ums)
                        {
                            pointer = (IntPtr)ums.PositionPointer;
                            bufferLength = ums.Length;
                        }
                        else
                        {
                            tempBuffer = Marshal.AllocHGlobal((int)bankData.Length);
                            pointer = tempBuffer;

                            bufferLength = bankData.Length;

                            using (var ustream = new UnmanagedMemoryStream((byte*)tempBuffer, bankData.Length))
                            {
                                bankData.CopyTo(ustream);
                            }
                        }*/

                        RESULT loadResult = FMOD.Studio.System.FMOD_Studio_System_LoadBankMemory(Instance.studioSystem.handle, ptr, (int)length, LOAD_MEMORY_MODE.LOAD_MEMORY, FMOD.Studio.LOAD_BANK_FLAGS.NORMAL, out loadedBank.Bank.handle);
        ...
        private static void LoadBank(TextAsset asset, bool loadSamples, string bankId)
        {
            ...
            else
            {
                ...
                if (asset.text.StartsWith(BankStubPrefix))
                {
                    ...
                    name = name.Replace("examples/Build/Desktop/", "");
